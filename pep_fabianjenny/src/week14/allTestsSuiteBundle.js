...
pmSuite.add("setQualifier", assert => {
    const attr1 = Attribute(".....", "Person.4711.firstname");
    const attr2 = Attribute("Dierk", "nothing");

    assert.is(attr1.getObs(VALUE).getValue(), "....."); // old known values have been updated on init
    assert.is(attr2.getObs(VALUE).getValue(), "Dierk"); // old known values have been updated on init

    attr2.setQualifier("Person.4711.firstname");        // setting the qualifier will sync the values
    assert.is(attr1.getObs(VALUE).getValue(), "Dierk");
    assert.is(attr2.getObs(VALUE).getValue(), "Dierk");

    attr1.setConvertedValue("Dierk");
    assert.is(attr1.getObs(VALUE).getValue(), "Dierk"); // all syncs are now in place
    assert.is(attr2.getObs(VALUE).getValue(), "Dierk");
});
